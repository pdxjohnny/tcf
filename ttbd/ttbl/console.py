...
class podman_pc(ttbl.power.socat_pc, generic_c):
    """Implement a connection to a podman container's stdin/stdout given the
    container name.

    A podman attach is executed so was to send anything written to
    the console to the process running in the container. Anything read is
    received and recorded for the console interface to send to clients.

    This class implements two interfaces:

    - power interface: to start a podman connection recorder in the
      background as soon as the target is powered on.
      ...
      Anything read form the podman connection is written to the
      *console-NAME.read* file and anything written to it is written
      to *console-NAME.write* file, which is sent to the remote
      machine.
    ...
    :params str container_name: Name of the container to attach to stdin and
      stdout of.

    Other parameters as to :class:`generic_c`.
    """
    def __init__(self, container_name, socat_pc_kwargs = None, **kwargs):
        assert isinstance(container_name, str)
        generic_c.__init__(self)
        if socat_pc_kwargs is None:
            socat_pc_kwargs = {}
        ttbl.power.socat_pc.__init__(
            self,
            "PTY,link=console-%(component)s.write,rawer"
            "!!CREATE:console-%(component)s.read",
            # --detach-keys="": Disable detach via key sequence
            # --sig-proxy=false: Do not proxy UNIX signals
            "EXEC:'podman attach --detach-keys='' --sig-proxy=false %(container_name)s'"
            ",sighup,sigint,sigquit",
            **socat_pc_kwargs
        )
        # pass those fields to the socat_pc templating engine
        self.kws['container_name'] = container_name
        self.paranoid_get_samples = 1
        self.upid_set(f"console over podman-attach to {container_name}",
                      name = f"podman-attach:{container_name}",
                      container_name = container_name)

    def on(self, target, component):
        podman_container_name = target.fsdb.get(
            "interfaces.console." + component + ".parameter_container_name", None)
        if podman_container_name:
            self.kws['container_name'] = podman_container_name
        ttbl.power.socat_pc.on(self, target, component)
        generation_set(target, component)
        generic_c.enable(self, target, component)
